...
class Settings():  #for easy future modification/s
    def __init__(self):
        self.name   = 'Word Search but in Space'
...
#art shortcuts
p.display.set_icon(p.image.load(asset('Logo')).convert_alpha())
...
def list_highs(text, lives, score, highest, done, word_list):  #processes display for combine mode
    ...
    if text!='' and text.lower() in word_list and text.lower() not in highest and text.lower() not in done:
        ...
        if len(highest)<6: 
            highest.append(text.lower())
            score = score + engine.scrabble_score(text.lower())
        else:
            while i < len(highest):
